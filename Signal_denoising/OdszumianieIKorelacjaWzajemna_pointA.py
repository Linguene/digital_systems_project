...
#PointA.wav
x = np.loadtxt('pointa.txt', dtype=float)
...
plt.plot(x_noisy)
plt.plot(x_denoise)
plt.xlabel("x signalA")
plt.ylabel("y signal A")
...
print('Przesunięcie(po odszumieniu) pointa-sent =', np.argmax(D) - (len(file_bD) - 1))
